...
function renderUserNav() {
  ...
  if (currentUser) {
    ...
    if (window.Teammates) Teammates.stopJoinRequestsPolling();
    closeMenuDrawer();
    container.innerHTML = `
  ...
  container.innerHTML = papers.map(p => `
    <div class="card">
      <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:0.5rem;">
        ...
        ${isAdmin ? `<button class="btn btn-sm btn-danger" style="padding:0.25rem 0.6rem; font-size:0.75rem; white-space:nowrap;" onclick="adminDeletePaper('${p.paper_id}')">Delete Paper</button>` : ''}
      </div>
...
async function adminDeletePaper(paperId) {
  if (currentUser?.role !== 'Admin') return alert('Only administrators can delete research papers.');
...
function openDirectChat(receiverId, receiverName) {
  ...
  window._activeReceiverName = receiverName || 'Classmate';
  
  const titleEl = document.getElementById('directChatUserName');
  if (titleEl) titleEl.innerText = 'Chat with ' + (receiverName || 'Classmate');
...
async function fetchDirectChatMessages() {
  const user = window.currentUser || (typeof currentUser !== 'undefined' ? currentUser : null) || JSON.parse(localStorage.getItem('rip_user') || 'null');
  if (!window._activeReceiverId || !user) return;
  
  try {
    const res = await fetch(`/api/messages/history?user1=${encodeURIComponent(user.user_id)}&user2=${encodeURIComponent(window._activeReceiverId)}`);
    const data = await res.json();
    const container = document.getElementById('directChatMessages');
    if (!container) return;
    
    if (!data.messages || data.messages.length === 0) {
      container.innerHTML = `<p style="text-align:center; color:var(--text-muted); font-size:0.85rem; margin-top:2.5rem;">No messages yet. Send a message below to start chatting!</p>`;
      return;
    }
    ...
    container.innerHTML = data.messages.map(m => {
      ...
      const senderName = mine ? 'You' : (m.sender_name || window._activeReceiverName || 'Classmate');
      const timeStr = m.timestamp ? m.timestamp.substring(11, 16) : '';
      const safeText = String(m.message_text || '')
        .replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');

      return `<div class="chat-bubble ${mine ? 'mine' : 'other'}">
        <div style="font-size:0.7rem; opacity:0.85; font-weight:700; margin-bottom:0.25rem;">${senderName}</div>
        <div style="word-break:break-word; font-size:0.88rem; line-height:1.4;">${safeText}</div>
        <div style="font-size:0.65rem; opacity:0.6; text-align:right; margin-top:0.25rem;">${timeStr}</div>
      </div>`;
    }).join('');
  ...
  try {
    const res = await fetch('/api/messages/send', {
      ...
      body: JSON.stringify({
        ...
        receiver_id: window._activeReceiverId,
        message_text: text
      })
    });
    const data = await res.json();
